...
namespace Library.Core
{
    ...
    public class AddUserControlViewModel : BaseViewModel
    {
        ...
        /// <summary>
        /// Tells the view if an invalid personal number is entered
        /// </summary>
        [DefaultValue(false)]
        public bool InvalidPNumber { get {

                // No need to show the message if there is no input
                if (PNumber.Length == 0) return false;

                // Personal number must be 10 digits
                if(PNumber.Length == 10 && long.TryParse(PNumber, out long _ ))
                {
                    if(DateTime.TryParseExact(PNumber.Substring(0, 6), 
                                              "yyMMdd", 
                                              CultureInfo.InvariantCulture, 
                                              DateTimeStyles.None,
                                              out DateTime d) && d < DateTime.Now) return false;
                }
                return true;
            } }
